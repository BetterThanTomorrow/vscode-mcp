...
- **Library:** start/stop socket, install wrapper, Cursor register/unregister, ECA `.eca/config.json` upsert, `initialize` / `tools/list` / `resources/*` / `ping` from the Copilot manifest
- **Consumer:** implement `tools/call`, pass `:mcp/on-request`, settings → `create-config`, workspace-stable ECA port file, when-contexts / commands / UI
...
| `create-config` | Merge opts with defaults (`:server/host` → `"127.0.0.1"`; `:mcp/auto-register-eca?` → `false`) |
...
| `stop!+` | Unregister Cursor (best-effort), stop socket |
